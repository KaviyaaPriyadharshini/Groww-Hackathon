...
- **Plain-words explanation with web-searched context**: for a surfaced stock, the drawer narrates the decomposition and any headlines found on NSE filings, Google News, and GDELT, through the same grounding validators as the briefing. When a Gemini key is present the explanation searches the live web; otherwise it falls back to an OpenRouter narrator and then a template.
- **Rule automation**: a saved rule can carry up to five actions, email or a signed webhook. Webhook payloads are HMAC-SHA256 signed, the URL is checked against private and loopback ranges at save time and again at fire time, and each action fires once per signal.
- **Anonymous by default, Google optional**: a session is a create-only bearer token, so nobody can log in as someone else by typing a name. Google sign-in layers an identity on top through OIDC with PKCE. A public landing page explains the thesis and offers a signed-out read-only demo.
- **More end-of-day detections**: alongside the live signals, the daily job records opening gaps, RSI(14) extremes, and SMA-20/50 crossovers as first-class events.

## Where AI is used, and where it is refused

Used: unsupervised clustering for peer groups, an LLM as narrator for the briefing, an LLM as compiler for rules, and a search-grounded LLM as narrator for per-stock explanations. In all of them the model never touches a price and never decides what is meaningful; every output is validated by code and falls back to a deterministic template.

The Gemini explanation path is the one place model input leaves for a source whose free tier may use prompts to improve its products. It is off unless `GOOGLE_API_KEY` is set, the payload carries only ticker symbols and computed numbers, and enabling billing on the Google project moves it to the paid terms where prompts are not retained.
...
Tests: `uv run pytest` (370 tests, needs the docker Postgres; provider and LLM calls are mocked at the HTTP boundary). `pytest -m integration` runs the migration test.

The compose file maps Postgres to host port 5433 and Redis to 6379, so `DATABASE_URL` points at `localhost:5433`. Google sign-in needs `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, and a redirect URI registered in the Google Cloud console as `http://localhost:8000/api/auth/google/callback`; without them the anonymous session flow still works. If the database is empty on startup the app seeds itself once under a Redis lock, so the explicit `--seed` step is optional after the first run.
...
| `APP_BASE_URL`, `API_BASE_URL` | Origins written into confirmation, unsubscribe, and OAuth redirect links. | Local defaults are fine; set to the public URLs when deployed. |
| `GOOGLE_API_KEY` | Web-search-grounded per-stock explanations via Gemini. Empty means the OpenRouter or template narrator. | aistudio.google.com/apikey. See the Gemini caveat below. |
| `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, `OIDC_REDIRECT_URI` | Google sign-in. Empty leaves anonymous sessions working. | Google Cloud console, OAuth Web client, redirect `http://localhost:8000/api/auth/google/callback`. |
...
Rejected: NSE quote endpoints (Akamai-blocked from datacenters), Alpha Vantage (returns valid-looking data for an invalid key), Stooq (JavaScript proof-of-work), NewsAPI free (forbids deployed use). Google Gemini's free tier is used only for the optional web-grounded explanation, with the data caveat noted above; it is never on the price or briefing path.
